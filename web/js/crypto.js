...
export async function setServerKeys(data) {
  ...
  for (const [day, key] of Object.entries(keys)) cryptoKeys.set(day, await createCryptoKey(key));
...
async function encryptWithWebCrypto(text) {
  ...
  const encrypted = await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv }, currentCryptoKey, encoder.encode(text));
  return { keyDay: currentKeyDay, text: JSON.stringify({ v: 1, alg: 'AES-GCM', iv: bytesToBase64(iv), data: bytesToBase64(new Uint8Array(encrypted)) }) };
}

async function decryptWithWebCrypto(payload, keyDay) {
  const key = cryptoKeys.get(keyDay) || currentCryptoKey;
  if (!key) return 'Не удалось расшифровать сообщение';
  const decrypted = await globalThis.crypto.subtle.decrypt({ name: 'AES-GCM', iv: base64ToBytes(payload.iv) }, key, base64ToBytes(payload.data));
...
async function createCryptoKey(passphrase) {
  ...
  return globalThis.crypto.subtle.deriveKey({ name: 'PBKDF2', salt: encryptionSalt, iterations: 120000, hash: 'SHA-256' }, baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
...
function encryptWithFallback(text) {
  ...
  return { keyDay: currentKeyDay, text: JSON.stringify({ v: 1, alg: 'SM-FALLBACK', iv: String(iv), data: bytesToBase64(encrypted) }) };
}
